...
@endsection
@section('content')
    @if (Route::currentRouteName() == 'type_crimes.create')
        <form action="{{ route('type_crimes.store') }}" method="post" enctype="multipart/form-data">
        @else
            <form action="{{ route('type_crimes.update', $type->uuid) }}" method="post" enctype="multipart/form-data">
                @method('PUT')
    @endif
    @csrf
    @include('pages.backoffice.type_crimes._form', ['btnAction' => $btnAction, 'type' => $type ])
    </form>
@stop
@section('js')
