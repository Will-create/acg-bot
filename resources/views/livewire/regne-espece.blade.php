    ...
    <div class="row">
        <div class="col-md-5">
            <div class="form-group">
                ...
                <option onclick="refreshSelectize()" value="animal"> Animal</option>
        ...
        <div class="col-md-5">
            <div>
                <label>   Espèce  <span class="text-danger">*</span></label>
                    <select onchange="document.getElementById('espece').value=this.value" style="width: 100%" id="mySelect2">
                        <option value="" selected > Séléctionnez une espèce</option>
                    @forelse ($especes as $espece)
                <option  class="selection"  value="{{$espece->id}}"> {{$espece->nom}}</option>
                    @empty
                    Aucune espèce
                    @endforelse
                      </select>
                    <input type="text" value="{{$crime->id}}">
                    <input type="text" value="{{$espece_id}}" id="espece" wire:model="espece_id">

                      @error('espece_id') <span class="error text-danger">{{ $message }}</span> @enderror
            </div>
        </div>
