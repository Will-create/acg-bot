		<!-- BACK-TO-TOP -->
		<a href="#top" id="back-to-top"><i class="fa fa-angle-up"></i></a>
		<!-- JQUERY JS -->
		<script src="<?php echo e(URL::asset('assets/js/jquery-3.4.1.min.js')); ?>"></script>
		<script src="<?php echo e(URL::asset('assets/plugins/datatable/jquery.dataTables.min.js')); ?>"></script>
		<!-- BOOTSTRAP JS -->
		<script src="<?php echo e(URL::asset('assets/plugins/bootstrap/js/bootstrap.bundle.min.js')); ?>"></script>
		<script src="<?php echo e(URL::asset('assets/plugins/bootstrap/js/popper.min.js')); ?>"></script>
		<!-- SPARKLINE JS-->
		<script src="<?php echo e(URL::asset('assets/js/jquery.sparkline.min.js')); ?>"></script>
		<!-- CHART-CIRCLE JS-->
		<script src="<?php echo e(URL::asset('assets/js/circle-progress.min.js')); ?>"></script>
		<!-- RATING STARJS -->
		<script src="<?php echo e(URL::asset('assets/plugins/rating/jquery.rating-stars.js')); ?>"></script>
		<!-- EVA-ICONS JS -->
		<script src="<?php echo e(URL::asset('assets/iconfonts/eva.min.js')); ?>"></script>
		<!-- INPUT MASK JS-->
		<script src="<?php echo e(URL::asset('assets/plugins/input-mask/jquery.mask.min.js')); ?>"></script>
		<!-- SIDE-MENU JS-->
		<script src="<?php echo e(URL::asset('assets/plugins/sidemenu/sidemenu.js')); ?>"></script>
		<!-- STICKY JS -->
		<script src="<?php echo e(URL::asset('assets/js/stiky.js')); ?>"></script>
		<!-- PERFECT SCROLL BAR js-->
		<!-- CUSTOM SCROLLBAR JS-->
		<script src="<?php echo e(URL::asset('assets/plugins/scroll-bar/jquery.mCustomScrollbar.concat.min.js')); ?>"></script>
         <?php echo $__env->yieldContent('js'); ?>
        	<!-- INTERNAL FORN WIZARD JS-->
		<script src="<?php echo e(URL::asset('assets/plugins/formwizard/jquery.smartWizard.js')); ?>"></script>
		<script src="<?php echo e(URL::asset('assets/plugins/formwizard/fromwizard.js')); ?>"></script>
		<!-- INTERNAL ACCORDION-WIZARD FORM JS -->
		<script src="<?php echo e(URL::asset('assets/plugins/accordion-Wizard-Form/jquery.accordion-wizard.min.js')); ?>"></script>
		<!--INTERNAL  ADVANCED FORM JS -->
		<script src="<?php echo e(URL::asset('assets/js/advancedform.js')); ?>"></script>
		<!-- SIDEBAR JS -->
		<script src="<?php echo e(URL::asset('assets/plugins/sidebar/sidebar.js')); ?>"></script>
		<!-- CUSTOM JS -->
		<script src="<?php echo e(URL::asset('assets/js/fakeLoader.min.js')); ?>"></script>
		<script src="<?php echo e(URL::asset('assets/js/custom.js')); ?>"></script><?php /**PATH /home/louisbertson/Desktop/criminalite/resources/views/layouts/footer-scripts.blade.php ENDPATH**/ ?>